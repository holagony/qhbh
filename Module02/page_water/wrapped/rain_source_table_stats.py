...
def table_stats_simple(data_df, ele):
    ...
    # 先计算降水资源量
    idx = data_df.index # 先保存原始的index
    data_df = pd.merge(data_df,sta_area,on='Station_Id_C')
    data_df['PRE_Time_2020'] = (data_df['PRE_Time_2020'] * data_df['面积']).round(1)
    data_df.drop(columns='面积', axis=1, inplace=True)
