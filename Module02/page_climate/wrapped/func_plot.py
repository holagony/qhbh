...
def add_north(ax, labelsize=18, loc_x=0.9, loc_y=0.99, width=0.03, height=0.075, pad=0.16):
    ...
    ax.text(s='N', x=minx + xlen * loc_x, y=miny + ylen * (loc_y - pad + height), fontsize=labelsize, horizontalalignment='center', verticalalignment='bottom')
...
def plot_and_save(shp_path, mask_grid, lon_grid, lat_grid, exp_name, insti_name, year_name, save_path):
    ...
    # 添加图例
    legend = ax.legend(handles=[province_handle, lakes_handle, state_handle, glaciers_handle], loc='lower left', fontsize=10, ncol=2, frameon=False, title='图例', title_fontsize=10, handletextpad=0.5, columnspacing=1.0, bbox_to_anchor=(0.0, 0.05))
