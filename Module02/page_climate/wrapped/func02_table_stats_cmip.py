...
def table_stats_simple_cmip(ds, stats_result_his,var, sta_list):
    ...
    stats_result['区域距平'] = ((stats_result.iloc[:, :].mean(axis=1) - stats_result_his.iloc[:-4:,-3].mean().mean()) * 100).round(1)
    stats_result['区域距平百分率'] = (stats_result['区域距平']/ stats_result_his.iloc[:-4:,-3].mean().mean()).round(2)
