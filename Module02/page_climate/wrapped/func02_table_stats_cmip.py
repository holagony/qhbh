...
def table_stats_simple_cmip(ds, stats_result_his,var, sta_list):
    ...
    tmp_df.loc['距平'] = (tmp_df.loc['平均'].astype('float') - stats_result_his.iloc[-3,1:-3:].astype('float')).round(1)
    tmp_df.loc['距平百分率'] = ((tmp_df.loc['距平'].astype('float') / stats_result_his.iloc[-3,1:-3:].astype('float')) * 100).round(2)
    # 合并所有结果
