...
def table_stats_rain(data_df, disaster):
    '''
    计算气候变化风险预估-降水风险
    '''
    data_df['危险性'] = 0.5*data_df['rx5day'] + 0.4*data_df['r20'] + 0.1*data_df['Alti']
    data_df['危险性'] = (data_df['危险性']-data_df['危险性'].min())/(data_df['危险性'].max()-data_df['危险性'].min()) # 0-1标准化
    data_df['危险性'].fillna(0, inplace=True)

    data_df = data_df.pivot_table(index=data_df.index, columns=['Station_Id_C'], values='危险性')  # 统计时段df
    data_df.index = data_df.index.strftime('%Y')
    
    disaster_val = disaster.disaster.data.reshape(1,-1) # 承载体插值到站点后的静态值
    data_df = data_df*disaster_val # 最后计算的风险值 0~1之间
...
if __name__ == '__main__':
    ...
    last_year = 2023
    stats_result, post_data_df, post_refer_df, regression = table_stats_rain(data_df, element_str)
