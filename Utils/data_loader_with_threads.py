...
def get_database_data(sta_ids, element_str, table_name, time_freq, stats_times):
    ...
    if "Station_Id_C" in element_str:
...
def get_database_result(sta_ids, elements, table_name, time_freq, stats_times,types,station,float_flag):
    '''
    从数据库获取数据
    '''
    conn = psycopg2.connect(database=cfg.INFO.DB_NAME, user=cfg.INFO.DB_USER, password=cfg.INFO.DB_PWD, host=cfg.INFO.DB_HOST, port=cfg.INFO.DB_PORT)
    ...
    if time_freq == 'Y':  # '%Y,%Y'
        ...
        if 12 in mon_list:
            query = sql.SQL(f"""
                            ...
                            """)
            cur.execute(query, (start_year, end_year,str(int(start_year)-1),str(int(end_year)+1), sta_ids,types))
            ...
            query = sql.SQL(f"""
                            ...
                            """)  
            cur.execute(query, (start_year, end_year, mon_, sta_ids,types))
        ...
        start_month = stats_times.split(',')[0][4:]
        end_month = stats_times.split(',')[1][4:]
        cur.execute(query, (start_year, start_month, start_year, end_year, end_year, end_month, sta_ids,types))
        data = cur.fetchall()
        ...
        start_date = stats_times.split(',')[0][6:]
        end_date = stats_times.split(',')[1][6:]
        cur.execute(query, (start_year, start_month, start_date, start_year, end_year, end_year, end_month, end_date, sta_ids,types))
        data = cur.fetchall()
        ...
        start_date = dates.split(',')[0][2:]
        end_date = dates.split(',')[1][2:]
        cur.execute(query, (start_year, end_year, start_mon, start_date, start_mon, end_mon, end_mon, end_date, sta_ids,types))
        data = cur.fetchall()
    ...
    result_df = df.pivot_table(index=df.index, columns=['Station_Id_C'], values='value',aggfunc='first')  # 近1年df
    # 关闭数据库
    cur.close()
    conn.close()
...
def get_grid_result(sta_ids, element_str, table_name, time_freq, stats_times):
    '''
    获取数据库里面格点数据结果，相关表：
    qh_climate_nc_cmpas_station
    qh_climate_nc_cldas_station
    qh_climate_nc_hrcldas_station
    datetime %Y%m%d%H
    '''
    elements = 'Station_Id_C,Lon,Lat,Datetime,' + element_str
    conn = psycopg2.connect(database=cfg.INFO.DB_NAME, user=cfg.INFO.DB_USER, password=cfg.INFO.DB_PWD, host=cfg.INFO.DB_HOST, port=cfg.INFO.DB_PORT)
    cur = conn.cursor()
    ...
    if time_freq == 'Y':  # '%Y,%Y'
        query = sql.SQL(f"""
                        ...
                        FROM public.{table_name}
                        WHERE
                            SUBSTRING(datetime, 1, 4) BETWEEN %s AND %s
                            AND station_id_c IN %s
                        """)
        ...
        mon_list = [str(mon_).zfill(2) for mon_ in stats_times[1].split(',')]  # 提取月份并补零
        mon_ = tuple(mon_list)
        years = stats_times[0]
        start_year = years.split(',')[0]
        end_year = years.split(',')[1]
        ...
        if '12' in mon_list:
            query = sql.SQL(f"""
                            ...
                            FROM public.{table_name}
                                WHERE (SUBSTRING(datetime, 1, 4) BETWEEN %s AND %s) 
                                AND SUBSTRING(datetime, 5, 2) IN ('12', '01', '02')
                                OR (SUBSTRING(datetime, 1, 4) = %s AND SUBSTRING(datetime, 5, 2) = '12')
                                OR (SUBSTRING(datetime, 1, 4) = %s AND SUBSTRING(datetime, 5, 2) IN ('01', '02'))
                                AND station_id_c IN %s
                            """)
            cur.execute(query, (start_year, end_year,str(int(start_year)-1),str(int(end_year)+1), sta_ids))
            ...
            query = sql.SQL(f"""
                            ...
                            WHERE
                                (SUBSTRING(datetime, 1, 4) BETWEEN %s AND %s
                                AND SUBSTRING(datetime, 5, 2) IN %s)
                                AND station_id_c IN %s
                            """)  
            cur.execute(query, (start_year, end_year, mon_, sta_ids))
        ...
        query = sql.SQL(f"""
                        ...
                        FROM public.{table_name}
                        WHERE
                            ((SUBSTRING(datetime, 1, 4) = %s AND SUBSTRING(datetime, 5, 2) >= %s)
                            OR (SUBSTRING(datetime, 1, 4) > %s AND SUBSTRING(datetime, 1, 4) < %s)
                            OR (SUBSTRING(datetime, 1, 4) = %s AND SUBSTRING(datetime, 5, 2) <= %s))
                            AND station_id_c IN %s
                        """)
        ...
        query = sql.SQL(f"""
                        ...
                        FROM public.{table_name}
                        WHERE
                            ((SUBSTRING(datetime, 1, 4) = %s AND SUBSTRING(datetime, 5, 2) >= %s AND SUBSTRING(datetime, 7, 2) >= %s)
                            OR (SUBSTRING(datetime, 1, 4) > %s AND SUBSTRING(datetime, 1, 4) < %s)
                            OR (SUBSTRING(datetime, 1, 4) = %s AND SUBSTRING(datetime, 5, 2) <= %s AND SUBSTRING(datetime, 7, 2) <= %s))
                            AND station_id_c IN %s
                        """)
        ...
        query = sql.SQL(f"""
                        ...
                        WHERE
                            (SUBSTRING(datetime, 1, 4) BETWEEN %s AND %s
                            AND (
                                (SUBSTRING(datetime, 5, 2) = %s AND SUBSTRING(datetime, 7, 2) >= %s)
                                OR (SUBSTRING(datetime, 5, 2) > %s AND SUBSTRING(datetime, 5, 2) < %s)
                                OR (SUBSTRING(datetime, 5, 2) = %s AND SUBSTRING(datetime, 7, 2) <= %s)
                            ))
                            AND station_id_c IN %s
                        """)
